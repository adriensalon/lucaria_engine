...
namespace detail {
    ...
    async_container<mesh_implementation> fetch_mesh_async(const std::filesystem::path& path)
    {
    ...
    mesh_object mesh_manager::fetch(const std::filesystem::path& path)
    {
        resource_container<mesh_implementation>* _cell = _resources.get_or_create_by_path(path, [&] {
            return fetch_mesh_async(path);
        });

        return mesh_object { _cell };
    }
...
{
    return detail::engine_assets().meshes.fetch(path);
}
...
mesh_object::mesh_object(detail::resource_container<detail::mesh_implementation>* cell)
    : _cell(cell)
{
}
